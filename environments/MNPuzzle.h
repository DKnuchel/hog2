...
public:
    ...
    void GetSuccessors(const MNPuzzleState<width, height> &stateID,
    ...
    void
    Draw(Graphics::Display &display, const MNPuzzleState<width, height> &l1, const MNPuzzleState<width, height> &l2,
         float v) const;
    ...
    **/
    static void Create_Random_MN_Puzzles(MNPuzzleState<width, height> &goal,
                                         std::vector<MNPuzzleState<width, height>> &puzzle_vector,
                                         unsigned num_puzzles);
    ...
    described above.
    **/
    static int read_in_mn_puzzles(const char *filename, bool first_counter, unsigned max_puzzles,
                                  std::vector<MNPuzzleState<width, height>> &puzzle_vector);
    ...
    **/
    ...
    void Set_Use_Linear_Conflict_Heuristic(bool to_use = true) {
        use_conflict = to_use;
        use_manhattan = true;
    }
...
template<int width, int height>
void
MNPuzzle<width, height>::GetActions(const MNPuzzleState<width, height> &stateID, std::vector<slideDir> &actions) const {
    actions.resize(0);
    for (unsigned int i = 0; i < operators[stateID.blank].size(); i++) {
...
template<int width, int height>
slideDir
MNPuzzle<width, height>::GetAction(const MNPuzzleState<width, height> &a, const MNPuzzleState<width, height> &b) const {
                ...
                if (state1.puzzle[x + y * width] != 0) {
                    ...
                    switch (weight) {
                        case kUnitWeight:
                            ...
                            break;
                        case kUnitPlusFrac:
                            man_dist += absDist * (1.0 + 1.0 / (1.0 + movingTile));
                            break;
                        case kSquared:
                            man_dist += absDist * (movingTile) * (movingTile);
                            break;
                        case kSquareRoot:
                            man_dist += absDist * sqrt(movingTile);
                            break;
 ...
 */
template<int width, int height>
double MNPuzzle<width, height>::ConflictH(const MNPuzzleState<width, height> &state1,
                                          const MNPuzzleState<width, height> &state2) const {
    double conflicts = 0;
    ...
    for (int r = 0; r < width; r++) {
        for (int cl = 0; cl < height; cl++) {
            for (int cr = cl + 1; cr < height; cr++) {
                if (state2.puzzle[r * height + cl] && state2.puzzle[r * height + cr] &&
                    r == pRow[state2.puzzle[r * height + cl]] &&
                    pRow[state2.puzzle[r * height + cl]] == pRow[state2.puzzle[r * height + cr]] &&
                    pCol[state2.puzzle[r * height + cl]] > pCol[state2.puzzle[r * height + cr]]) {
                    conflicts++;
                }
            }
        }
    ...
    for (int c = 0; c < width; c++) {
        for (int ru = 0; ru < height; ru++) {
            for (int rd = ru + 1; rd < height; rd++) {
                if (state2.puzzle[ru * height + c] && state2.puzzle[rd * height + c] &&
                    c == pCol[state2.puzzle[ru * height + c]] &&
                    pCol[state2.puzzle[ru * height + c]] == pCol[state2.puzzle[rd * height + c]] &&
                    pRow[state2.puzzle[ru * height + c]] > pRow[state2.puzzle[rd * height + c]]) {
                    conflicts++;
                }
            }
        }
...
template<int width, int height>
double
MNPuzzle<width, height>::GCost(const MNPuzzleState<width, height> &a, const MNPuzzleState<width, height> &b) const {
    // Options:
    // * tile squared
    // square root of tile
    // tile itself
    switch (weight) {
        case kUnitWeight:
            return 1;
        case kUnitPlusFrac:
            return (1.0 + 1.0 / (1.0 + a.puzzle[b.blank]));
        case kSquared:
            return a.puzzle[b.blank] * a.puzzle[b.blank];
        case kSquareRoot:
            return sqrt(a.puzzle[b.blank]);
        case kSquarePlusOneRoot:
            return sqrt(1 + a.puzzle[b.blank] * a.puzzle[b.blank]);
    }
//	if (weighted)
//		return a.puzzle[b.blank]*a.puzzle[b.blank];
...
double MNPuzzle<width, height>::AdditiveGCost(const MNPuzzleState<width, height> &s, const slideDir &d) const {
    switch (d) {
        case kLeft:
            return InPattern(s.puzzle[s.blank - 1]) ? GCost(s, d) : 0;
        case kUp:
            return InPattern(s.puzzle[s.blank - width]) ? GCost(s, d) : 0;
        case kDown:
            return InPattern(s.puzzle[s.blank + width]) ? GCost(s, d) : 0;
        case kRight:
            return InPattern(s.puzzle[s.blank + 1]) ? GCost(s, d) : 0;
...
double MNPuzzle<width, height>::GCost(const MNPuzzleState<width, height> &s, const slideDir &d) const {
    switch (weight) {
        ...
        case kUnitPlusFrac: {
            switch (d) {
                case kLeft:
                    return 1.0 + 1.0 / (1.0 + s.puzzle[s.blank - 1]);
                case kUp:
                    return 1.0 + 1.0 / (1.0 + s.puzzle[s.blank - width]);
                case kDown:
                    return 1.0 + 1.0 / (1.0 + s.puzzle[s.blank + width]);
                case kRight:
                    return 1.0 + 1.0 / (1.0 + s.puzzle[s.blank + 1]);
        ...
        case kSquared: {
            switch (d) {
                case kLeft:
                    return s.puzzle[s.blank - 1] * s.puzzle[s.blank - 1];
                case kUp:
                    return s.puzzle[s.blank - width] * s.puzzle[s.blank - width];
                case kDown:
                    return s.puzzle[s.blank + width] * s.puzzle[s.blank + width];
                case kRight:
                    return s.puzzle[s.blank + 1] * s.puzzle[s.blank + 1];
        ...
        case kSquareRoot: {
            switch (d) {
                case kLeft:
                    return sqrt(s.puzzle[s.blank - 1]);
                case kUp:
                    return sqrt(s.puzzle[s.blank - width]);
                case kDown:
                    return sqrt(s.puzzle[s.blank + width]);
                case kRight:
                    return sqrt(s.puzzle[s.blank + 1]);
        ...
        case kSquarePlusOneRoot: {
            switch (d) {
                case kLeft:
                    return sqrt(1 + s.puzzle[s.blank - 1] * s.puzzle[s.blank - 1]);
                case kUp:
                    return sqrt(1 + s.puzzle[s.blank - width] * s.puzzle[s.blank - width]);
                case kDown:
                    return sqrt(1 + s.puzzle[s.blank + width] * s.puzzle[s.blank + width]);
                case kRight:
                    return sqrt(1 + s.puzzle[s.blank + 1] * s.puzzle[s.blank + 1]);
...
bool MNPuzzle<width, height>::GoalTest(const MNPuzzleState<width, height> &state,
                                       const MNPuzzleState<width, height> &theGoal) const {
...
void MNPuzzle<width, height>::Draw(Graphics::Display &display, const MNPuzzleState<width, height> &s) const {
    ...
    display.FrameRect({xOrigin, yOrigin, xOrigin + (width) * squareSize, yOrigin + (height) * squareSize},
                      Colors::lightgray, 0.04 * squareSize);
    for (unsigned int y = 0; y < height; y++) {
        for (unsigned int x = 0; x < width; x++) {
            if (s.puzzle[x + y * width] != 0) {
                sprintf(txt, "%d", s.puzzle[x + y * width]);
                display.FillRect({xOrigin + x * squareSize, yOrigin + y * squareSize, xOrigin + (x + 1) * squareSize,
                                  yOrigin + (y + 1) * squareSize}, Colors::white);
                display.FrameRect({xOrigin + x * squareSize, yOrigin + y * squareSize, xOrigin + (x + 1) * squareSize,
                                   yOrigin + (y + 1) * squareSize}, Colors::darkblue, 0.01 * squareSize);
                if (s.puzzle[x + y * width] > 0)
                    display.DrawText(txt,
                                     {xOrigin + x * squareSize + squareSize / 2.f,
                                      yOrigin + y * squareSize + squareSize / 2.f},
                                     Colors::blue, squareSize / 2.0f, Graphics::textAlignCenter,
...
slideDir MNPuzzle<width, height>::GetAction(const MNPuzzleState<width, height> &s, point3d p) {
    ...
    // Find which location was hit
    {
        ...
        for (unsigned int y = 0; y < height; y++) {
            for (unsigned int x = 0; x < width; x++) {
                if (s.puzzle[x + y * width] != 0) {
                    Graphics::rect r = {xOrigin + x * squareSize, yOrigin + y * squareSize,
                                        xOrigin + (x + 1) * squareSize, yOrigin + (y + 1) * squareSize};
...
template<int width, int height>
void MNPuzzle<width, height>::Draw(Graphics::Display &display, const MNPuzzleState<width, height> &s1,
                                   const MNPuzzleState<width, height> &s2, float v) const {
    float squareSize = std::max(width, height) + 1;
    squareSize = 2.0f / squareSize;
    float xOrigin = (0 - ((float) width) / 2.0f) * squareSize;
    float yOrigin = (0 - ((float) height) / 2.0f) * squareSize;
        ...
        for (unsigned int x = 0; x < width; x++) {
            if (s1.puzzle[x + y * width] == s2.puzzle[x + y * width]) {
                if (s1.puzzle[x + y * width] != 0) {
                    ...
                    display.FillRect(
                            {xOrigin + x * squareSize, yOrigin + y * squareSize, xOrigin + (x + 1) * squareSize,
                             yOrigin + (y + 1) * squareSize}, Colors::white);
                    display.FrameRect(
                            {xOrigin + x * squareSize, yOrigin + y * squareSize, xOrigin + (x + 1) * squareSize,
                             yOrigin + (y + 1) * squareSize}, Colors::darkblue, 0.01 * squareSize);
                    if (s1.puzzle[x + y * width] > 0)
                        display.DrawText(txt, {xOrigin + x * squareSize + squareSize / 2.f,
                                               yOrigin + y * squareSize + squareSize / 2.f}, Colors::blue,
                                         squareSize / 2.0f, Graphics::textAlignCenter, Graphics::textBaselineMiddle);
                }
                ...
                sprintf(txt, "%d", s1.puzzle[x + y * width]);
                display.FillRect(
                        {p1.x * v + p2.x * (1 - v), p1.y * v + p2.y * (1 - v), p1.x * v + p2.x * (1 - v) + squareSize,
                         p1.y * v + p2.y * (1 - v) + squareSize}, Colors::white);
                display.FrameRect(
                        {p1.x * v + p2.x * (1 - v), p1.y * v + p2.y * (1 - v), p1.x * v + p2.x * (1 - v) + squareSize,
                         p1.y * v + p2.y * (1 - v) + squareSize}, Colors::darkblue, 0.01 * squareSize);

                if (s1.puzzle[x + y * width] > 0)
                    display.DrawText(txt, {p1.x * v + p2.x * (1 - v) + squareSize / 2.f,
                                           p1.y * v + p2.y * (1 - v) + squareSize / 2.f}, Colors::blue,
                                     squareSize / 2.0f, Graphics::textAlignCenter, Graphics::textBaselineMiddle);
            }
        }
    }
...
template<int width, int height>
void MNPuzzle<width, height>::OpenGLDraw(const MNPuzzleState<width, height> &s1, const MNPuzzleState<width, height> &s2,
                                         float v) const {
    glEnable(GL_LINE_SMOOTH);
    for (unsigned int y = 0; y < height; y++) {
        for (unsigned int x = 0; x < width; x++) {
            ...
            if (s1.puzzle[x + y * width] == s2.puzzle[x + y * width]) {
                ...
                switch (GetAction(s1, s2)) {
                    case kUp:
                        DrawTile(x, (y - 1) * v + (y) * (1 - v), c1, c2, width, height);
                        break;
                    case kDown:
                        DrawTile(x, (y + 1) * v + (y) * (1 - v), c1, c2, width, height);
                        break;
                    case kLeft:
                        DrawTile((x) * (1 - v) + (x - 1) * v, y, c1, c2, width, height);
                        break;
                    case kRight:
                        DrawTile((x + 1) * v + (x) * (1 - v), y, c1, c2, width, height);
                        break;
...
int MNPuzzle<width, height>::read_in_mn_puzzles(const char *filename, bool puzz_num_start, unsigned int max_puzzles,
    ...
    PermutationPuzzle::PermutationPuzzleEnvironment<MNPuzzleState<width, height>, slideDir>::Read_In_Permutations(
            filename, width * height, max_puzzles, permutations, puzz_num_start);
...
template<int width, int height>
GraphPuzzleDistanceHeuristic<width, height>::GraphPuzzleDistanceHeuristic(MNPuzzle<width, height> &mnp, Graph *graph,
                                                                          int count)
...
MNPuzzleState<width, height> MNPuzzle<width, height>::Generate_Random_Puzzle() {
    ...
    std::vector<int> permutation = PermutationPuzzle::PermutationPuzzleEnvironment<MNPuzzleState<width, height>, slideDir>::Get_Random_Permutation(
            width * height);
...
uint64_t MNPuzzle<width, height>::GetStateHash(const MNPuzzleState<width, height> &s) const {
    ...
    for (unsigned int x = 0; x < width * height - 2; x++) {
        hashVal += locs[x] *
                   PermutationPuzzle::PermutationPuzzleEnvironment<MNPuzzleState<width, height>, slideDir>::Factorial(
                           numEntriesLeft - 1) / 2;
        numEntriesLeft--;
...
template<int width, int height>
void MNPuzzle<width, height>::Create_Random_MN_Puzzles(MNPuzzleState<width, height> &goal,
                                                       std::vector<MNPuzzleState<width, height>> &puzzle_vector,
                                                       unsigned num_puzzles) {
