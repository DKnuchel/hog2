...
void PhO()
{
	...
	//Testing:
	start = STP::GetKorfInstance(1);
	PhOHeuristic<width, height, MNPuzzleState<width, height>, slideDir, MNPuzzle<width, height>> pho(&hPairs, &hTriplets, goal);
	
	
	pho.HCost(start, goal);
	
	Heuristic<MNPuzzleState<width, height>> h;
	h.lookups.resize(0);
	h.lookups.push_back({kDynNode, 0, 0});
	h.heuristics.resize(0);
	h.heuristics.push_back(&pho);
