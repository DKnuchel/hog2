...
public:
    IDAStar() {
        useHashTable = usePathMax = false;
        storedHeuristic = false;
    }
...
void IDAStar<state, action, verbose>::GetPath(SearchEnvironment<state, action> *env,
    ...
    std::cout << "initial: " << rootH << std::endl;
...
double IDAStar<state, action, verbose>::DoIteration(SearchEnvironment<state, action> *env,
    ...
    for (unsigned int x = 0; x < neighbors.size(); x++) {
        ...
        if (env->GoalTest(thePath.back(), goal) &&
            flesseq(g + edgeCost, bound)) //check that a solution that does not exceed the bound was found
            return 0;
        thePath.pop_back();
        // pathmax
